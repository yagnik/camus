...
public class TimebasedEventMapper extends Mapper<LongWritable, Text, LongWritable, Text> {
    ...
    protected void setup(Context context) throws IOException, InterruptedException {
        ...
        this.statsdEnabled = context.getConfiguration().getBoolean(ShopifyEtlRecordReader.STATSD_ENABLED, false);
        jsonStringMessageDecoder.init(props, "default topic");
        if (this.statsdEnabled) {
            statsd = new NonBlockingStatsDClient(
                    "Camus",
                    ShopifyEtlRecordReader.getStatsdHost(context),
                    ShopifyEtlRecordReader.getStatsdPort(context),
